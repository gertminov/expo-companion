...
class CanvasBoox(
    context: Context,
    var initialBmp: Bitmap? = null,
    private var background: Bitmap?,
    initialPageIdx: Int = 0,
    ...
    private val PEN_UP_REFRESH_RATE = 800

    private val bmpPaint = Paint(Paint.DITHER_FLAG)
    private val bmpPaintWithBG =
        Paint(Paint.DITHER_FLAG).apply { xfermode = PorterDuffXfermode(PorterDuff.Mode.MULTIPLY) }
    ...
    /**
     * a list of functions that depend on the Drawsurface to be ... there (dimensions > 0)
     * The will be executed in the surfaceCreated function
     */
    private var needsSurface: List<()-> Unit> = listOf()
    ...
    fun redo(count: Int) {
        if (redoCount != count) {
            redoCount = count
            BitmapUtil.bitmapLock.withLock {
    ...
    private val layoutChangedListener: OnLayoutChangeListener by lazy {
        OnLayoutChangeListener { v, left, top, right, bottom, oldLeft, oldTop, oldRight, oldBottom ->
            ...
            touchHelper.setPenUpRefreshEnabled(false)
            touchHelper.setPenUpRefreshTimeMs(PEN_UP_REFRESH_RATE)
        }
    }
            ...
            override fun surfaceCreated(holder: SurfaceHolder) {
                ...
                bitmap?.let { bmp ->
                    holder.lockCanvas()?.let { lockCanvas ->
                        ...
                        val paint = getBackGroundPaint(lockCanvas)
                        lockCanvas.drawBitmap(bmp, 0f, 0f, paint)
                        holder.unlockCanvasAndPost(lockCanvas)
                    }
                    true
                } ?: cleanSurfaceView()
                needsSurface.forEach { function -> function() }
                needsSurface = listOf()
            }
    ...
    private fun getBackGround(): Bitmap? {
        if (hasBackground) {
            background = background ?: run {
                return BackgroundGenerator.dots(width, height)
            }
        }
        return background
    }

    fun getBackGroundPaint(canvas: Canvas): Paint {
        return getBackGround()?.let { bg ->
            canvas.drawBitmap(bg, 0f, 0f, bmpPaint)
            bmpPaintWithBG
        } ?: bmpPaint
    }
...
//            generateBG()
    ...
    private fun drawOrErasePointsToBitmap(
        ...
        val (undo, redo) = bitmapLock.withLock {
    ...
    fun penOrEraser(isPen: Boolean) {
        if (isPen == isPencil)
            return
        //if the Drawsurface is not initialized yet,
        // the change tool function is added to the list of functions that depend on the surface
        if (width <= 0 && height <= 0) {
            needsSurface.plus {
                changeTool(isPen)
            }
    ...
    private fun changeTool(isPen: Boolean) {
        if (isPen) {
            pencil()
        } else {
            eraser()
    ...
    private fun cleanSurfaceView(): Boolean {
        ...
        initialBmp?.let {
            val paint = getBackGround()?.let { bg ->
                canvas.drawBitmap(
                    bg,
